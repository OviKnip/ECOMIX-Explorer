...
ui <- page_navbar(
  ...
  nav_panel(title = "Site Details",
            ...
            # Site switcher + background + a small locator map, all in one
            # compact row instead of three stacked blocks, so a laptop-height
            # viewport doesn't burn ~150px on the selector alone.
            layout_columns(
              col_widths = c(3, 6, 3),
              selectInput(
              ...
              card(
                ...
                leafletOutput("site_mini_map", height = "150px")
              )
            ),
            ...
            # Both cards cap their default (visible) height well below a
            # typical laptop viewport and scroll/expand internally - the
            # underlying plots keep their full, legibly-spaced size and are
            # just windowed down, and full_screen = TRUE on both cards gives
            # a one-click larger view when that's not enough.
            layout_columns(
              col_widths = c(6, 6),
              card(
                full_screen = TRUE,
                ...
                div(
                  style = "position: relative; height: 480px; overflow-y: auto;",
                  plotOutput(
                    "site_chem_grid",
                    height = "900px",
              ...
              card(
                ...
                tags$small(
                  class = "text-muted",
                  "Defaults to the four chemicals with the greatest concentration range at this site - change any selector below to plot something else."
                ),
                layout_columns(
                ...
                plotOutput("site_time_series", height = "480px")
              )
            )),
